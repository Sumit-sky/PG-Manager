...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $existSql = "SELECT * FROM $pgnam WHERE `aadharno` =  '$aadharno'";
    $result = mysqli_query($conn, $existSql);
    $numExistRows = mysqli_num_rows($result);

    $existSql2 = "SELECT * FROM $pgnam WHERE `email` =  '$email'";
    $result2 = mysqli_query($conn, $existSql2);
    $numExistRows2 = mysqli_num_rows($result2);

    $existSql3 = "SELECT * FROM $pgnam WHERE `phoneno` =  '$phonenumber'";
    ...
    if ($numExistRows > 0 || $numExistRows2 > 0 || $numExistRows3 > 0) {
        ...
        echo "<script>alert('Email/Phone Number already exists');</script>";
    } else {
        $sql = "INSERT INTO $pgnam VALUES('$pgname','$roomno','$roomrent','$aadharno','$fullname','$email','$fathername',$phonenumber,'$pphonenumber','$joindate','$address','$gender','$destination')";
        $result = mysqli_query($conn, $sql);

        if ($result) {
            move_uploaded_file($_FILES["profilepic"]["tmp_name"], "ProfilePictures/" . $_FILES["profilepic"]["name"]);
            $showAlert = true;
            echo "<script>window.alert('Data Added Successfully !'); window.location.href='add_rentee.php';</script>";
        } else {
...
<head>
    ...
    <title>Add Rentee's</title>
    <link rel="stylesheet" href="add_rentee.css">
...
<body>
    ...
    <div class="center">
        <form action="add_rentee.php" method="post" enctype="multipart/form-data">
            <div class="form-header">

                <h1>Add Rentee Data</h1>
            </div>
            <div class="user-details">
                ...
                <div class="txt_field">
                    ...
                    <input type="file" id="profilepic" name="profilepic" accept="image/*" style="opacity:1;z-index:-1;" required>

                </div>

                <div class="txt_field">
                    <input type="tel" step="1" pattern="\d+" id="roomno" name="roomno" minlength="3" maxlength="8"
                        required>
                    <label for="roomno">Room Number</label>
                </div>

                <div class="txt_field">
                    <input type="tel" id="roomrent" step="1" pattern="\d+" name="roomrent" minlength="4" maxlength="9"
                        required>
